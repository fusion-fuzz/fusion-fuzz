"""
projects/spidermonkey/parser.py — turn JavaScript files into seeds.

Beyond the variables/dataflows every fusion strategy consumes, this
records what a jit-test file's `// |jit-test|` directive line declares:

  flags           shell flags the test needs (--ion-eager and friends).
  allow_oom /     failures the corpus declares *correct* for this test.
  expects_error   Carrying them through matters as much as the flags: a
                  test that is supposed to throw is not a finding when it
                  throws, and one that is supposed to run out of memory is
                  not a finding when it does.
  needs_load      the file load()s a jit-test lib/ helper that a fused
                  seed no longer sits next to.
...
try:
    from projects.spidermonkey.analyzer import analyze_seed
except ImportError:  # pragma: no cover - direct-load fallback
    _spec = importlib.util.spec_from_file_location(
        "ffl_sm_analyzer_parser",
...
class SpiderMonkeyParser(BaseParser):
    extensions = ['.js', '.mjs']
    ...
    def parse_content(self, content, filename=""):
        ...
        return {
            # core/dryrun.py's _COLLECTORS keys on this. SpiderMonkey and
            # V8 are the same language, so they share the collector, the
            # lexicon and the fusion strategies; only the driver and the
            # crash oracle differ.
            "type":           "javascript",
            "extension":      ".js",
            "flags":          facts["flags"],
            "allow_oom":      facts["allow_oom"],
            "allow_unhandlable_oom": facts["allow_unhandlable_oom"],
            "expects_error":  facts["expects_error"],
            "exit_status":    facts["exit_status"],
            "is_module":      facts["is_module"],
            "uses_shell_fns": facts["uses_shell_fns"],
            "jit_score":      facts["jit_score"],
...
_parser = SpiderMonkeyParser(__file__)
...
if __name__ == "__main__":
    ...
    print("Executing SpiderMonkey parser standalone.")
