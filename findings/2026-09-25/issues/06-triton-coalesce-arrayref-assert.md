# [Triton] `-tritongpu-coalesce` asserts `Index < Length` on a module with a `#ttg.linear` layout (cuda:100)
...
```
triton-opt: llvm/ADT/ArrayRef.h:247:
  const T &llvm::ArrayRef<long>::operator[](size_t) const [T = long]:
  Assertion `Index < Length && "Invalid index!"' failed.
```

`triton-opt` parses and verifies the module (running it with no passes is clean); the
single pass `-tritongpu-coalesce` aborts.
...
```bash
triton-opt coalesce.mlir -allow-unregistered-dialect -tritongpu-coalesce -o /dev/null
```

## Expected

Either coalesce the module or reject it with a diagnostic. The `#ttg.linear` layout in the
reproducer has `warp = [[0, 0], [0, 0]]`, which the pass appears to index past.

## Notes

Found compiling Triton's own test modules for Blackwell (`ttg.target = "cuda:100"`) with
the module's `ttg.num-warps` and `threads-per-warp` attributes preserved. One of the two
parent modules also asserts on its own under this pass, so the fusion is not required.
